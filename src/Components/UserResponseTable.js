...
function UserResponseTable() {
  ...
  return (
    <>
      ...
      <h1>Suggestion Feedback Data</h1>
      <div></div>
      <table className="table">
        ...
        <tbody>
          {suggestiondata.map((item, index) => (
            <tr key={index}>
              <td>{index + 1}</td>
              <td>{item.name && typeof item.name === 'object' ? `${item.name.firstName} ${item.name.lastName}` : item.name}</td>
              <td>{item.email}</td>
              <td>
               
                {item.businessName && (
                  <div><b>Business Name:</b> {item.businessName}</div>
                )}
                {item.details && <div><b>Details:</b> {item.details}</div>}
                {item.improvementAction && (
                  <div><b>Improvement Action:</b> {item.improvementAction}</div>
                )}
                {item.receiveResponse && (
                  <div><b>Receive Response:</b> {item.receiveResponse}</div>
                )}
                 {item.query ||
